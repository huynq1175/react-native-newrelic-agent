...
const LINKING_ERROR =
  `The package 'react-native-newrelic-agent' doesn't seem to be linked. Make sure: \n\n` +
  Platform.select({ ios: "- You have run 'pod install'\n", default: '' }) +
  '- You rebuilt the app after installing the package\n' +
  '- You are not using Expo managed workflow\n';

const NewrelicAgent = NativeModules.NewrelicAgent
  ? NativeModules.NewrelicAgent
  : new Proxy(
      {},
      {
        get() {
...
export function recordHandledException(
  ...
  if (!error) error = new Error('Unknown exception');
  const jSException = pareJSException(error);
  return NewrelicAgent.recordHandledException(jSException, attributes);
}
...
export function crashNow(message?: string) {
  return NewrelicAgent.crashNow(message);
}
...
export async function startInteraction(
  ...
  return await NewrelicAgent.startInteraction(interactionName);
}
...
export function setInteractionName(interactionName: string) {
  return NewrelicAgent.setInteractionName(interactionName);
}
...
export function endInteraction(id: InteractionId) {
  return NewrelicAgent.endInteraction(id);
}
...
export function recordMetric(
  ...
  return NewrelicAgent.recordMetric(name, category, params);
}
...
export function setAttribute(
  ...
  return NewrelicAgent.setAttribute(name, { value });
}
...
export function setAttributes(attributes: EventAttributes) {
  return NewrelicAgent.setAttributes(attributes);
}
...
export function removeAttribute(name: string) {
  return NewrelicAgent.removeAttribute(name);
}
...
export function setUserId(userId: string) {
  return NewrelicAgent.setUserId(userId);
}
...
export function recordBreadcrumb(name: string, attributes?: EventAttributes) {
  return NewrelicAgent.recordBreadcrumb(name, attributes);
}
...
export function recordCustomEvent(
  ...
  return NewrelicAgent.recordCustomEvent(eventType, eventName, attributes);
}
...
export function noticeNetworkRequest(url: string, options: RequestOptions) {
  ...
  return NewrelicAgent.noticeNetworkRequest(url, attributes);
}
...
export function noticeNetworkFailure(url: string, options: RequestOptions) {
  ...
  return NewrelicAgent.noticeNetworkFailure(url, attributes);
}
